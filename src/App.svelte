<script lang="ts">
  import { type RaidHelper, raidHelpers, type RaidHelperResult, type RaidType, type AdditionalFields } from "./Helper";
    ...
    if (fields) {
      for (const [field, label] of Object.entries(fields)) {
        if (!additionalFields[field]) {
          return { success: false, error: `Please fill out the ${label} field.` };
        }
      }
    }

    return raidHelpers[raidType].calculate(diskCount, diskSize, additionalFields);
  });

  const selectedRaidHelper = $derived<RaidHelper | null>(raidType && raidHelpers[raidType] ? raidHelpers[raidType] : null);
</script>

<main class="w-[90%] max-w-[1100px] m-auto">
  <h1 class="w-full py-[2em] text-primary-400 text-center">super cool raid calculator 🌴🥥</h1>
  <div class="w-full flex flex-col lg:flex-row">
    <div class="w-full flex flex-col justify-between items-center lg:w-1/2">
      <div class="w-full flex justify-between mb-3 items-center">
        ...
        <input class="w-1/2" type="number" id="diskCount" bind:value={diskCount} min="1" max="64" />
      </div>

      <div class="w-full flex justify-between mb-3 items-center">
        <label for="diskSize">Disk Size (TB)</label>
        <input class="w-1/2" type="number" id="diskSize" bind:value={diskSize} min="1" max="50" />
      </div>
      ...
      {#if selectedRaidHelper?.additionalFields}
        {#each Object.entries(selectedRaidHelper.additionalFields) as [field, label]}
          <div class="w-full flex justify-between mb-3 items-center">
            ...
            <input class="w-1/2" type="number" id={field} bind:value={additionalFields[field]} min="0" />
          </div>
        {/each}
      {/if}
    </div>
